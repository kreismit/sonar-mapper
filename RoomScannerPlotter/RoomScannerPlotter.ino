...
SoftWire Wire = SoftWire(); // instantiate SoftWire class
Plotter p; // instantiate Plotter class
...
// false --> show on plotter
double x, y, z; // vars for position outputs
int r; // raw range, not correcting for angular offset
int d = 30; // number of milliseconds to delay

bool console=false; // Enable (true) or disable (false) debugging output
...
void setup() {
  ...
  if(error){ Serial.println("Failed to start the MB1242.");}
  if(console){
    Serial.begin(115200);
  ...
  int i; int samples = 100; tic = millis(); float dttotal=0;
  ...
  if(console){
    ...
    Serial.println("Roll\tPitch\tYaw\tRange");
  }
  else{
    p.Begin();
    p.AddXYGraph( "Room Scan Plot", 1000, "x (cm)", x, "y (cm)", y );
  }
  tic = millis();
}
...
void loop() {
  ...
  if(console){
    ...
    // Plotter output
    if(r < 765 && r != 0){ // if SONAR isn't maxed out
      // Serial.println( (String) "DATA," + yaw + "," + range + "," + x + "," + y);
      p.Plot();
    }
  }
  toc = millis();
